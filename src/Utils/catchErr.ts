...
const CatchErr = (err:{code?:string}) => {
    ...
    else if(code === "auth/email-already-in-use") toastErr("email already exist")
    else if(code === "auth/requires-recent-login") toastErr("logout and login before updating your profile")
   else if(code === "unavailable") toastErr("firebase client is offline")
   else if (code === "auth/invalid-credential")toastErr("invalid credential")
